...
| **A.5** | **Delegate reliability & Cowork interaction (FOUNDER-CRITICAL, 2026-07-22)** | Inserted between Phase A merge and Phase B. Trigger: Delegate reported a task complete that was not, with no evidence and no way to continue the conversation. Non-negotiable rule: **completion is determined by the orchestration system from explicit evidence and acceptance criteria — never model-declared.** Scope: full Delegate/Bodhi audit (why the false completion was possible — prompts, state transitions, status mapping, error swallowing, missing validators/artifact checks); normalized task-state model (queued/planning/awaiting_user_input/awaiting_approval/running/verifying/partially_completed/blocked/failed/cancelled/completed); structured completion-evidence record; persistent per-task conversation thread that never closes on the final response; configurable completion modes (system-verified / completed-pending-review / user-accepted); bounded execution limits; restart/resume without repeating consequential actions; **immediate trust patch first** (completion validator, partial/blocked states, honest status language, Beta verification warning); adversarial false-completion test set with a **zero false-completion target** on deterministic cases. Deliverables: DELEGATE_CURRENT_STATE_AUDIT.md, DELEGATE_TRUST_AND_COMPLETION_MODEL.md, DELEGATE_PERSISTENT_CONVERSATION_ARCHITECTURE.md, reuse map, migration + commit plan. Extend Bodhi/tasks/executor — no parallel task engine. No Cowork-comparison claims until benchmarks justify them. Phase B embeddings resume only after the trust patch ships and the architecture is approved |
