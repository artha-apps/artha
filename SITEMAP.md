...
| `src/agent/contextGather.ts` | `gatherContext()` — pre-`<think>` local context assembly: top-5 memories by semantic similarity scored against CACHED per-row embeddings (`memory_entities.embedding`, 1 query embed per turn; keyword fallback) + last-3-turn recap + active scopes → `<context>` block + `contextScore`. Excludes the recency-preamble top-20 (no double injection). Exports `embedText`/`memoryEmbeddingText`/`backfillMemoryEmbeddings` for the memory write paths |
...
| **router/** | |
| `src/router/benchmark.ts` | Model capability probes (plan / synthesis heuristics; `tool_args` is a REAL structured tool call scored by `scoreToolCallReply`, `benchmark.probe.test.ts`) that score local Ollama models for the model router; `benchmarkModel()` probes ONE model (post-install Model Fit fill-in), `runBenchmark()` sweeps the fleet |
...
| `src/router/modelProvisioner.ts` | Artha installs its own agent model (slice 2, v0.4.27): `planProvision()` decides from route source / installed tags / consent (`settings_json.autoModelManagement`, default on) / free disk / 24 h backoff; `ensureAgentModel()` pulls the tier default in the background, registers it (not active), runs the real tool-call probe, and emits `agent:modelProvision` + `llm:pullProgress` (`modelProvisioner.test.ts`) |
| `src/llm/ollamaPull.ts` | Shared streaming `/api/pull` client (`pullOllamaModel`, `isModelInstalled`) — handles Ollama's 200-with-error-line and verifies via `/api/tags`; used by the provisioner |
| `src/agent/escalation.ts` | Act-loop recovery when a turn produced nothing (slice 3): `nextEscalation()` restores withheld MCP tools after a discarded turn, else steps up one rung on `escalationLadder()` (larger eligible local models, user's oversized local pick last, never cloud); each remedy once per run (`escalation.test.ts`) |
| `src/agent/autoApprove.ts` | Plan approval by blast radius, not category: reversible plans (no deletes) run without the modal when `settings_json.autonomousActions` ≠ false; the run card/toast says so and Undo covers it (`autoApprove.test.ts`) |
| `src/mcp/toolBudget.ts` | Which MCP tools a run is offered: hides MCP tools that shadow a built-in (`move_file` vs `fs_move_file`), offers a server's tools only when the goal names it / it already worked in this chat / the skill allowlists it; the full set is kept for escalation (`toolBudget.test.ts`) |
| **ipc/** | |
...
| `src/llm/ollamaVersion.ts` | Pure Ollama version helpers — `parseVersion`/`compareVersions`/`versionSatisfies` (catalog `minOllamaVersion` gate BEFORE a pull), `isOutdatedPullError` (recognises Ollama's 412 "requires a newer version" text), `getServerVersion` (GET /api/version). Unit-tested (`ollamaVersion.test.ts`) |
| `src/llm/ollamaRuntimeManager.ts` | Artha-managed Ollama runtime — install/update Ollama THROUGH Artha: curated pin (`artha.space/ollama-runtime.json`, bundled fallback `BUNDLED_PIN_VERSION`, forward-only), official GitHub release asset per platform/arch, SHA-256 verified against the release's `sha256sum.txt` BEFORE extraction, unpacked with the OS `tar` into `<userData>/ollama-runtime/versions/<v>/`, `current.json` pointer (must stay inside the root), one previous version kept for rollback. Never touches the user's own Ollama install. End-to-end tested with a fake release (`ollamaRuntimeManager.test.ts`) |
| **mcp/** | |
| `src/mcp/registry.ts` | `MCPRegistry` — manages MCP server processes, tool schemas, invocations; injects decrypted connector credentials via `spawnEnv` (augmented PATH) and records per-server `conn_status`; tracks per-server call health and quarantines a server that failed `MCP_QUARANTINE_AFTER` calls with no successes (`onQuarantine` → OS notification, Retry reconnects); `getBuiltInToolSchemas()` / `getMcpServerTools()` feed the tool budget |
...
| `src/components/ModelStatusBanner.tsx` | Quiet bottom-left notice for the local-model startup flow — subscribes to `model:status`; shows "Starting/Warming…" while Artha auto-starts Ollama + warms the model, auto-dismisses on ready, persistent "install Ollama" (now installs THROUGH Artha via `OllamaRuntimeUpdater`; ollama.com is the secondary link) / error+retry states |
| `src/components/OllamaRuntimeUpdater.tsx` | Install/update the Ollama engine through Artha — drives `ollama:runtimeInstall` (download progress with cancel, SHA-256 verify, unpack) then the consent-gated `ollama:runtimeSwitch`; `card` variant = the Models → Browse "Ollama engine" card, `inline` variant = startup banner + onboarding. Shows "done" only after the :11434 server reports the new version |
| `src/components/WorkingIndicator.tsx` | "Artha is working…" cue — window-edge accent glow + a bottom-center pill while the agent is acting (`isStreaming`). Pre-first-token phase shows the orchestrator's live `agent:status` line ("Planning the steps…", "Plan: a → b → c") instead of a generic spinner. Pill is `z-[90]` so it stays visible over modals. The baseline in-app signal; desktop control + browser pane add their own specific cues |
...
| `public/ollama-runtime.json` | Curated Ollama release pin consumed by installed apps (`packages/app/src/llm/ollamaRuntimeManager.ts`) — bumping it rolls every app's "Update Ollama" target forward (never below the bundled pin) without an Artha release |
